...
minimax gameBoard markers currentPlayer depth
  ...
  | otherwise =
      getMaxOrMin (parMap rdeepseq applyMinMax nextBoardStates) markers currentPlayer
      where nextPlayer = getOtherPlayer markers currentPlayer
...
getScores gameBoard markers =
  ...
  where nextBoardStates = getNextBoardStates gameBoard (ai markers)
        applyMinMax boardState = minimax boardState markers (player markers) 1
...
getBestMove :: Board -> Markers String String -> String
getBestMove gameBoard markers =
  spotWithMaxScore !! 0
  where scoredBoard = assignScores gameBoard markers
        spotWithMaxScore = getMax scoredBoard
